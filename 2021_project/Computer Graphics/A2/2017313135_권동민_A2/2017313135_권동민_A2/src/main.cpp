...
void update()
{
	...
	GLint uloc;
	uloc = glGetUniformLocation(program, "color_type");	if (uloc > -1) glUniform1i(uloc, color);
...
void render()
{
	...
	for( auto& c : circles )
	{
		...
		if(b_index_buffer)	glDrawElements( GL_TRIANGLES, LA*LO*6, GL_UNSIGNED_INT, nullptr );
		else				glDrawArrays( GL_TRIANGLES, 0, LA*LO*6 ); // NUM_TESS = N
	}
...
void update_tess()
{
	...
	unit_circle_vertices = create_circle_vertices(LA, LO);
